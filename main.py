...
import info
import keyboard
import story_products_functions as spf
...
# ========= ПОДКЛЮЧЕНИЕ К БД И СОЗДАНИЕ ТАБЛИЦЫ Products ==================
# spf.initiate_db()       # подключились к БД и создали таблицу Products
# lst_products = [('Продукт 1', 'Описание 1', 100), ('Продукт 2', 'Описание 2', 200), ('Продукт 3', 'Описание 3', 300), ('Продукт 4', 'Описание 4', 400),]
# spf.insert_multiple_lines(lst_products)       # добавили в БД список продуктов
# products = spf.get_all_products()             # получили зи БД все записи продуктов
# =========================================================================
...
async def start(message: types.Message):
    ...
    msg = (f'Привет {message["from"]["first_name"]}! Этот бот поможет тебе узнать больше о здоровом питании, '
           f'а также рассчитать суточную норму потребления калорий, '
           f'ну и конечно же купить продукты для здорового питания!')
...
async def get_buying_list(message):
    ...
    products = spf.get_all_products()       # получим список всех продуктов из БД
    i = 1
    for product in products:
        s = f'Название: {product[0]} | Описание: {product[1]} | Цена: {product[2]}'
...
async def send_calories(message, state):
    ...
    try:
        ...
        await message.answer('\nДля показа меню введите команду "/start"')
